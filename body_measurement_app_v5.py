"""
AI-Based Body Measurement Web App (v5)
----------------------------------------
Stack: Streamlit (UI) + OpenCV (image handling) + MediaPipe (pose + segmentation)

WHAT'S NEW IN v5 (on top of everything v4 already fixed):
...
def page_measurements():
    ...
    st.markdown(
        ...
        "margin), plain background, good lighting, camera held level at chest height, about "
        "2-2.5 meters back. For the side photo, turn 90° and stand the same distance from the "
        "camera - keep a small gap between your arm and your torso (rest your hand slightly "
        "forward or bend the elbow a touch) so your arm doesn't overlap your torso outline."
    )
    ...
    with front_col:
        ...
        # FRONT SHOT 1
        # -------------------------
        if st.session_state.front_image_1 is None:
            front_image_1 = timer_camera_input(
                "Front view - shot 1", seconds=5, key="front1"
            )
            if front_image_1 is not None:
                st.session_state.front_image_1 = front_image_1
                st.rerun()
            st.info("Capture your first front-facing photo to continue.")
            return
        else:
            st.success("✅ Front shot 1 captured and stored.")
        ...
        if st.session_state.use_multi_shot:
            if st.session_state.front_image_2 is None:
                st.caption("Shot 1 is stored. Reposition slightly (or stay in the same position), then take shot 2.")
                front_image_2 = timer_camera_input(
                    "Front view - shot 2", seconds=5, key="front2"
                )
                if front_image_2 is not None:
                    st.session_state.front_image_2 = front_image_2
                    st.rerun()
                return
            else:
                st.success("✅ Front shot 2 captured and stored.")
            ...
            # FRONT SHOT 3
            # -------------------------
            if st.session_state.front_image_3 is None:
                st.caption("Shot 2 is stored. Reposition slightly, then take the final shot 3.")
                front_image_3 = timer_camera_input(
                    "Front view - shot 3", seconds=5, key="front3"
                )
                if front_image_3 is not None:
                    st.session_state.front_image_3 = front_image_3
                    st.rerun()
                return
            else:
                st.success("✅ Front shot 3 captured and stored.")
    ...
    with side_col:
        ...
        if st.session_state.side_image is None:
            side_image = timer_camera_input("Side view", seconds=5, key="side")
            if side_image is not None:
                st.session_state.side_image = side_image
                st.rerun()
        else:
            st.success("✅ Side photo captured and stored.")
